...
async def test_tenants():
    ...
    respx.get(f"{apiUrl}tenants/{tid}").respond(
        status_code=200, json=jsonResp["tenant"]
    )
...
async def test_instances():
    respx.get(f"{apiUrl}instances", params={"tenantId": tid}).respond(
        status_code=200, json=jsonResp["instancesByTenant"]
    )
    respx.get(f"{apiUrl}instances").respond(status_code=200, json=jsonResp["instances"])
    respx.post(f"{apiUrl}instances").respond(
        status_code=200, json=jsonResp["createInstance"]
    )
    respx.post(f"{apiUrl}instances/sizing").respond(
        status_code=200, json=jsonResp["instanceSizing"]
    )
    respx.get(f"{apiUrl}instances/{iid}").respond(
        status_code=200, json=jsonResp["instance"]
    )
    respx.delete(f"{apiUrl}instances/{iid}").respond(
        status_code=202, json=jsonResp["deleteInstance"]
    )
    respx.post(f"{apiUrl}instances/{iid}/pause").respond(
        status_code=200, json=jsonResp["pauseInstance"]
    )
    respx.post(f"{apiUrl}instances/{iid}/resume").respond(
        status_code=200, json=jsonResp["resumeInstance"]
    )
    respx.post(f"{apiUrl}instances/{iid}/overwrite").respond(
        status_code=200, json=jsonResp["overwriteInstance"]
    )
...
async def test_snapshots():
    respx.get(f"{apiUrl}instances/{iid}/snapshots").respond(
        status_code=200, json=jsonResp["snapshots"]
    )
    respx.post(f"{apiUrl}instances/{iid}/snapshots").respond(
        status_code=200, json=jsonResp["createSnapshot"]
    )
    respx.post(f"{apiUrl}instances/{iid}/snapshots/snapshot3/restore").respond(
        status_code=202, json=jsonResp["restoreSnapshot"]
    )
    respx.get(f"{apiUrl}instances/{iid}/snapshots?date=2024-06-30").respond(
        status_code=200, json=jsonResp["snapshotsByDate"]
    )
    respx.get(f"{apiUrl}instances/{iid}/snapshots/snapshot2").respond(
        status_code=200, json=jsonResp["snapshot"]
    )
...
async def test_customer_managed_keys():
    respx.get(f"{apiUrl}customer-managed-keys", params={"tenantId": tid}).respond(
        status_code=200, json=jsonResp["customerManagedKeysForTenant"]
    )
    respx.get(f"{apiUrl}customer-managed-keys").respond(
        status_code=200, json=jsonResp["customerManagedKeys"]
    )
    respx.get(f"{apiUrl}customer-managed-keys/cmk1").respond(
        status_code=200, json=jsonResp["customerManagedKey"]
    )
    respx.post(f"{apiUrl}customer-managed-keys").respond(
        status_code=202, json=jsonResp["createCustomerManagedKey"]
    )
...
async def test_exceptions():
    ...
    respx.post(f"{baseUrl}oauth/token").respond(
        status_code=401, json=jsonResp["authError"]
    )
    async with AuraClient(clientId, clientSecret) as client:
        try:
            resp = await client.instances()
    ...
    async with AuraClient(clientId, clientSecret) as client:
        # Test Auth Expired
        respx.get(f"{apiUrl}instances").respond(
            status_code=401, json=jsonResp["exception"]
        )
        try:
            resp = await client.instances()
            assert not resp
        ...
        # Test Bad Request
        respx.get(f"{apiUrl}instances").respond(
            status_code=400, json=jsonResp["exception"]
        )
        try:
            resp = await client.instances()
            assert not resp
        ...
        # Test Missing Data
        respx.get(f"{apiUrl}instances/{iid}").respond(
            status_code=404, json=jsonResp["exception"]
        )
        try:
            resp = await client.instance(iid)
            assert not resp
        ...
        # Test Rate Limit
        respx.get(f"{apiUrl}instances/{iid}").respond(
            status_code=429, json=jsonResp["exception"]
        )
        try:
            resp = await client.instance(iid)
            assert not resp
        ...
        # Test Other Error
        respx.get(f"{apiUrl}instances/{iid}").respond(
            status_code=418, json=jsonResp["exception"]
        )
        try:
            resp = await client.instance(iid)
            assert not resp
